export const TEST_MERCHANT_CONFIG = {
  MerchantID: '2000132',
  HashKey: '5294y06JbISpM5x9',
  HashIV: 'v77hoKGq4kWxNNIS',
  ReturnURL: 'https://api.test.com/our/hook',
  PaymentInfoURL: 'https://api.test.com/payment/info',
};

export const TEST_BASE_PARAMS = {
  MerchantTradeNo: 'testno123456',
  MerchantTradeDate: '2021/04/17 10:55:26',
  TotalAmount: 1000,
  TradeDesc: 'test trade description',
  ItemName: 'test item name',
};

/*
 * Remote Request Testing
 */

export const QueryTradeNo = {
  CreditOneTime: 'test25174199894103', //'4e5c882cab5a4796be7f',
  CreditDivide: 'SG0000001547Xadbcfce',
  CreditPeriod: '20211026001969730',
  CreditFlexible: 'f76a43953fc546519ae',
  WebATM: '20220426133333',
  ATM: 'N22042600020513',
  CVS: '05677f89acc348939d3', // '2204220310222',
  BARCODE: '1501222204252113159',
};
